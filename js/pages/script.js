...
document.getElementById("append_div").addEventListener("click", (e) => {
  ...
  const right_icon_delete =
    right_icon_closest?.querySelector(".right_icon_delete");

  right_icon_closest
    ?.querySelector(".right_hidden_div")
...
function updateStreakState(idx, state_habit, diff) {
  if (diff === 0) {
    ...
    if (state_habit.isChecked && !state_habit.isStreakChecked) {
      ...
      updateHabit(idx, {
        lastCheckedAt: state_habit.currDate,
        isStreakChecked: true,
        streak: state_habit.streak,
        lastCheckedAt: state_habit.lastCheckedAt,
        needWarning: false,
      });
    }
    // works
  ...
  if (diff === 1) {
    if (elem.isChecked && elem.isStreakChecked) {
      ...
      updateHabit(i, {
        ...
        isStreakChecked: elem.isStreakChecked,
      });
    }
  }
}
...
function editElement(e) {
  ...
  edit_btn.addEventListener("click", () => {
    ...
    update_state({
      habit: state.habit.map((iterable, i) =>
        idx === i
          ? {
              ...iterable,
...
function updateCheckbox(e) {
  ...
  updateHabit(idx, {
    isChecked: state.habit[idx].isChecked,
...
function isComplete() {
  ...
  document.getElementById(
    "completed"
  ).textContent = `${completed}/${state.total}`;
}
...
function render() {
  ...
  if (state.habit && state.habit.length > 0) {
    ...
    append_div
      .querySelectorAll(".wrapper-div-class")
...
function toggleCheckbox(check_uncheck, card) {
  if (check_uncheck) {
    card
      .querySelector(".left_icon")
      .setAttribute("data-lucide", "square-check");
    card.classList.add(
      "bg-green-200/20",
      "border-green-300/60",
...
function create_card(titleText, descriptionText, categoryText, st, idx) {
  ...
  const wrapperDiv2 = create_element("div", [
    "flex",
    "justify-between",
    "items-center",
  ]);
  ...
  const left_i = createIcon(
    ["data-lucide", "square"],
    ["w-5", "h-5", "left_icon", "dark:text-white"]
  );

  const left_h1 = create_element("h1", [
    "text-md",
    "font-semibold",
    "left_h1",
    "dark:text-white",
  ...
  const right_inner_div = create_element("div", [
    "flex",
    "gap-2",
    "items-center",
    "justify-center",
  ...
  // streak div
  const streak_div = create_element("div", [
    "rounded-2xl",
    "px-3",
    "py-0.5",
    "flex",
  ...
  const streak_icon = createIcon(
    ["data-lucide", "flame"],
    ["w-4", "h-4", "streak_icon"]
  );
  streak_icon.setAttribute("stroke", "red");

  const streak = create_element("h1", [
    "text-sm",
    "font-semibold",
    "dark:text-white/80",
    "streak_no",
  ...
  const category_div = create_element("div", [
    "category_div",
    "text-md",
    "font-semibold",
    "border",
  ...
  const category_p = create_element("p", [
    "category_p",
    "text-sm",
    "font-semibold",
    "dark:text-white/90",
  ...
  const right_icon_div = create_element("div", [
    "right_icon_div",
    "flex",
    "items-end",
    "p-1",
  ...
  const right_icon = createIcon(
    ["data-lucide", "ellipsis"],
    ["hover:bg-gray-50", "dark:text-white"]
  );
  const right_hidden_div = create_element("div", [
    "right_hidden_div",
    "hidden",
    "bg-white",
    "shadow-lg",
  ...
  const right_hidden_edit_div = create_element("div", [
    "right_icon_edit",
    "flex",
    "justify-start",
    "items-center",
  ...
  const right_hidden_edit_icon = createIcon(
    ["data-lucide", "square-pen"],
    ["w-4", "h-4"]
  );
  const right_hidden_div_edit_p = create_element("h1", [
    "text-md",
    "font-semibold",
  ]);
  ...
  const right_hidden_delete_div = create_element("div", [
    "right_icon_delete",
    "flex",
    "justify-start",
    "items-center",
  ...
  const right_hidden_delete_icon = createIcon(
    ["data-lucide", "trash"],
    ["w-4", "h-4"]
  );
  const right_hidden_div_delete_p = create_element("h1", [
    "text-md",
    "font-semibold",
  ]);
  right_hidden_div_delete_p.textContent = "Delete";
  right_hidden_delete_div.append(
    right_hidden_delete_icon,
    right_hidden_div_delete_p
  );
  ...
  // descrition div
  const description_div = create_element("div", [
    "text-md",
    "text-gray-700/80",
    "font-semibold",
    "my-1",
  ]);
  const description_p = create_element("p", [
    "description_p",
    "dark:text-white",
  ]);
